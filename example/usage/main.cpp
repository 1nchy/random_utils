...
template <typename _T1, typename _T2> std::ostream& operator<<(std::ostream& _os, const std::pair<_T1, _T2>& _p) {
    _os << '(' << _p.first << ", " << _p.second << ')';
    return _os;
}

int main() {
    icy::random_object<std::pair<const unsigned, std::string>> _ro;
    for (size_t _i = 0; _i < 10; ++_i) {
        const std::pair<unsigned, std::string>& _p = _ro.rand(
